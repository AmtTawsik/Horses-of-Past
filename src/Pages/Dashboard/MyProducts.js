...
const MyProducts = () => {
  ...
  useEffect(() => {
    axios
      .get(`https://horses-of-past-server.vercel.app/products/${user?.email}`)
      .then((res) => {
  ...
  const handleAdvertize = (_id) => {
    fetch(`https://horses-of-past-server.vercel.app/products/${_id}`, {
      method: "PUT",
    })
      .then((res) => res.json())
      .then((data) => {
          ...
          <tbody>
            {myProducts.map((myProduct, idx) => (
              <tr key={idx}>
                ...
                <td>
                  <span className="text-3xl">{myProduct.productName}</span>
                </td>
                <td>
                  <span className="text-3xl">${myProduct.resalePrice}</span>
                </td>
                <td>
                ...
                {myProduct.isAvailable ? (
                  <td>
                    ...
                    >
                      {myProduct.isAdvertized ? "Advertized" : "Make Advertize"}
                    </button>
                  </td>
                ) : (
                  <td className="text-red-500 text-3xl font-bold">
                    Not Available
                  </td>
                )}
                <td>
